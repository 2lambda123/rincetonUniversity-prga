...
for i, con in enumerate(cons):
    for j, c in enumerate(con):
        builder.connect(
            in_[c], builder.instances["cluster", i].pins["i"][j % k])
    ...
    if module.name == "top":
        ...
        for x in range(module.width):
            for y in range(module.height):
                if i := module.instances.get(((x, y), Corner.southwest)):
                    yield i
                if i := module.instances.get((x, y)):
                    yield i
                if i := module.instances.get(((x, y), Corner.northwest)):
                    yield i
            for y in reversed(range(module.height)):
                if i := module.instances.get(((x, y), Corner.northeast)):
                    yield i
                if i := module.instances.get(((x, y), Corner.southeast)):
                    yield i
    else:
...
visited, next_, flist = [clbtile.key], [
    ctx.database[ModuleView.design, clbtile.key]], []
while next_:
    d = next_.pop(0)
...
visited, next_, flist = [top.key], [
    ctx.database[ModuleView.design, top.key]], []
while next_:
    d = next_.pop(0)
    flist.append(getattr(d, "verilog_src", d.name + ".v"))
    for sub in d.instances.values():
...
with open("pinout.txt", "w") as f:
    ...
    for io, id_ in zip(ctx.summary.ios[1:], reversed(range(2, 34))):
        f.write(tmpl.format(pin=id_, x=io.position.x,
                y=io.position.y, z=io.subtile))
